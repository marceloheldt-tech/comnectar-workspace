...
async function fetchAll() {
  ...
  while (hasNext) {
    ...
    if (result.errors) { console.error(JSON.stringify(result.errors, null, 2)); process.exit(1); }
...
fs.writeFileSync(new URL('./robson_all.json', import.meta.url), JSON.stringify(products, null, 2));
console.log('Salvo em scripts/robson_all.json');
